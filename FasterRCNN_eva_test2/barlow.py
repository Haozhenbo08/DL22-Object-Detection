...
class BarlowTwins(nn.Module):
    def __init__(self):
        super().__init__()
        ## Change the backbone to ResNet50
        self.backbone = torchvision.models.resnet50(pretrained=False)
        self.backbone.fc = nn.Identity()

        ## Change the sizes to 2048
        # projector
        sizes = [2048] + list(map(int, '2048-2048-2048'.split('-')))
        layers = []
    ...
    def forward(self, y1, y2):
        ...
        ## Here, not sure about this 1024's meaning
        c.div_(1024)
...
def getTrainedBarlowModel(totalEpochs):
    ...
    path = 'barlow-50-test01'
    batch_size = 1024
    train_dataset = dataset.UnlabeledDataset(root='/unlabeled', transform=Transform())

    # build a PyTorch dataloader
    loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=4)
    ...
    if os.path.isfile('./barlow-50-test01/checkpoint.pth'):
        ckpt = torch.load(f'./{path}/checkpoint.pth',
                        map_location='cpu')
    ...
    #                 print(json.dumps(stats), file=stats_file)
        ...
        if running_loss < least_loss:
            ...
            torch.save(state, f'./{path}/checkpoint.pth')
        running_loss = 0
    ...
    torch.save(model.backbone.state_dict(),
            f'./{path}/model_dict.pth')
    torch.save(model,
            f'./{path}/model.pth')

    
    return model.backbone.state_dict(),model
